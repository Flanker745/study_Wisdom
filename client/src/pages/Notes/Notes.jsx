...
const Notes = () => {
  ...
  const fetchNotes = async () => {
    try {
      let response = await fetch(`${api}/viewNotes`, {
        method: "GET",
        headers: {
  ...
  const searchNotes = async (query) => {
    setLoading(true);
    try {
      let url = `${api}/searchNotes?search=${encodeURIComponent(query)}`;
  ...
  const Notespage =  location.pathname.startsWith("/notes");
  useEffect(() => {
    if (searchInput ) {
        ...
        <div className="grid grid-cols-1 w-[90%] m-auto sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {notes.map((note) => (
            <div
              ...
              onClick={() => handleNoteClick(note._id)} // Trigger navigation on click
            >
